"""22:00 job — copy matching topopassport.topotext labels into odh_export.ogh-disruption.

Read-only SELECT from mggt_asu. Reprojection MSK-77 (SRID 980077) → EPSG:4326
...
JOB_NAME = "ogh_disruption_topotext"
BOOTSTRAP_LIMIT = 50
SOURCE_SRID = OGH_ANALIZ_SOURCE_SRID
FILTER_PASS = "topotext"
QUALIFIED_TABLE = 'odh_export."ogh-disruption"'
REMOTE_SCHEMA = "topopassport"
REMOTE_TABLE = "topotext"
FETCH_SIZE = 250
...
ILIKE_PATTERNS: tuple[str, ...] = tuple(f"%{keyword}%" for keyword in TEXT_KEYWORDS)
...
def remote_select_sql(*, last_fid: int, limit: int | None) -> sql.Composed:
    query = sql.SQL(
        """
        SELECT fid, text, {number}, ST_AsEWKB({geometry})
        FROM {schema}.{table}
        WHERE text IS NOT NULL
          AND btrim(text) <> ''
          AND {geometry} IS NOT NULL
          AND {number} IS NOT NULL
          AND btrim({number}::text) <> ''
          AND text ILIKE ANY(%s)
          AND fid > %s
        """
    ).format(
        number=sql.Identifier("Number"),
...
def _fetch_remote_rows(
    ...
    *,
    ...
    params: list[Any] = [list(ILIKE_PATTERNS), last_fid]
    if limit is not None:
        params.append(limit)
    rows: list[tuple[Any, ...]] = []
    with remote_conn.cursor(name="ogh_disruption_topotext") as cur:
...
def sync_topotext() -> SyncResult:
    ...
    with mggt_asu_connection() as remote_conn:
    ...
    with local_connection() as local_conn:
        with local_conn.cursor() as cur:
            for fid, text, number, geom_ewkb in rows:
                label_text = normalize_label(text)
                source_json = normalize_number(number)
