"""Fetch photo metadata from MSI Holes API for genplan.uuid_api UUIDs."""
...
from collector.genplan_uuids import load_uuid_api_uuids_pending_meta
from collector.msi_holes_client import MsiHolesClient

logger = logging.getLogger(__name__)

JOB_NAME = "genplan_fetch_uuid_api"
...
def run() -> None:
    ...
    with local_connection() as conn:
        run_id = log_job_run(
            ...
            JOB_NAME,
            "running",
            f"{len(pending_uuids)} uuid_api uuid(s) pending meta fetch",
        )

    if not pending_uuids:
        message = "0 uuid_api uuid(s) pending meta fetch"
        with local_connection() as conn:
            log_job_run(
                conn,
                JOB_NAME,
        ...
        logger.info("genplan_fetch_uuid_api finished: %s", message)
        return

    fetch_uuids = pending_uuids
    if GENPLAN_FETCH_UUID_API_LIMIT > 0:
        fetch_uuids = pending_uuids[:GENPLAN_FETCH_UUID_API_LIMIT]
        logger.info(
            "GENPLAN_FETCH_UUID_API_LIMIT=%s: processing %s of %s pending uuid(s)",
            GENPLAN_FETCH_UUID_API_LIMIT,
            len(fetch_uuids),
            len(pending_uuids),
        )
    ...
    try:
        with MsiHolesClient(
            ...
            for uuid in fetch_uuids:
                try:
                    ...
                    upsert_photo_meta(meta, source=f"fetch_uuid_api:{uuid}")
                    meta_saved += 1
        ...
        logger.exception("genplan_fetch_uuid_api job failed")
        with local_connection() as conn:
            log_job_run(
                conn,
                JOB_NAME,
