...
Формат JSON совпадает с ответом MSI Holes API (раздел «Получение данных по имеющейся фотографии»), но наш endpoint **принимает** данные (`PUT`), а не отдаёт (`GET`).
...
| Прод-сервер | `172.21.198.219` (данные пишутся сюда) |

**Статус (2026-08-03):** публичный API **работает** (`GET /health` → `{"status":"ok"}`).
...
Только для отладки. Смежникам — **только** прод-домен.

## Что нужно получить от администратора MONITOR

1. **Base URL:** `https://monitor-crm.mggt.ru`
...
```bash
export MONITOR_BASE_URL="https://monitor-crm.mggt.ru"
...
with MonitorClient(
    base_url="https://monitor-crm.mggt.ru",
...
| Правило | Описание |
|---------|----------|
| Идемпотентность | Повторный `PUT` meta с тем же uuid обновляет запись |
...
| 409 | Для uuid-endpoint: уже существует |
...
| Адрес (прод) | `https://m2m.msi-holes.cxm.dev` | `https://monitor-crm.mggt.ru` |
